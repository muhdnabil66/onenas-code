...
brew install anomalyco/tap/onenas # macOS 和 Linux（推荐，始终保持最新）
brew install onenas              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S onenas            # Arch Linux (Stable)
paru -S onenas-bin               # Arch Linux (Latest from AUR)
mise use -g onenas               # 任意系统
nix run nixpkgs#onenas           # 或用 github:muhdnabil66/onenas-code 获取最新 dev 分支
```
...
onenas 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/muhdnabil66/onenas-code/releases) 或 [onenas.ai/download](https://onenas.atlasflux.my/download) 下载。
...
1. `$onenas_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.onenas/bin` - 默认备用路径
...
onenas 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://onenas.atlasflux.my/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://onenas.atlasflux.my/docs)。
...
如果你在项目名中使用了 “onenas”（如 “onenas-dashboard” 或 “onenas-mobile”），请在 README 里注明该项目不是 onenas 团队官方开发，且不存在隶属关系。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/onenas)
