...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["onenas.json", "onenas.jsonc", "opencode.json", "opencode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".onenas", ".opencode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".onenas" || path.basename(item) === ".opencode")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter(
      (item) => path.basename(item) !== ".onenas" && path.basename(item) !== ".opencode",
    ).toReversed()
