...
// etc.).
//
// The visual language mirrors the ONeNas Code app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `ONeNas Code is now connected to ${escapeHtml(provider)}.` : "ONeNas Code is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `ONeNas Code couldn't finish connecting to ${escapeHtml(provider)}.`
        : "ONeNas Code couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from ONeNas Code.",
    }),
  })
}
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("ONeNas Code couldn't finish connecting to "+PROVIDER+"."):"ONeNas Code couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from ONeNas Code."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("ONeNas Code is now connected to "+PROVIDER+"."):"ONeNas Code is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// ONeNas Code wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="ONeNas Code" role="img">
