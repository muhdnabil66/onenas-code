  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "onenes alpha")
        draft.active = active === "opencode" ? "onenes alpha" : typeof active === "string" ? active : "onenes alpha"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "onenes alpha"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "onenes alpha")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["onenes alpha"], store.mode)
    })
