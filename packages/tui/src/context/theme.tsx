  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "onenas alpha")
        draft.active = active === "opencode" ? "onenas alpha" : typeof active === "string" ? active : "onenas alpha"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "onenas alpha"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "onenas alpha")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["onenas alpha"], store.mode)
    })
