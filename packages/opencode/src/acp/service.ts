...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `onenas auth login` in the terminal",
      name: "Login with onenas",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, AtlasFlux provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const atlasfluxProvider = providers[ProviderV2.ID.make("atlasflux")]
  const atlasfluxModel = atlasfluxProvider ? Provider.sort(Object.values(atlasfluxProvider.models))[0] : undefined
  if (atlasfluxProvider && atlasfluxModel) return { providerID: atlasfluxProvider.id, modelID: atlasfluxModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "ONeNas Code prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "ONeNas Code service failure", service })
}
